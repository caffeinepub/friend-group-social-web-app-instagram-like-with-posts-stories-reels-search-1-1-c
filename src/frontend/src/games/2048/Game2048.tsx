    ...
    <div className="space-y-4 py-4">
      <div className="text-center">
        ...
        <p className="text-sm text-muted-foreground">
          Use buttons or arrow keys to play
        </p>
      </div>
      <div className="grid grid-cols-4 gap-2 max-w-md mx-auto bg-muted p-2 rounded-lg">
        {board.map((row, i) =>
          row.map((cell, j) => (
            <div
              // biome-ignore lint/suspicious/noArrayIndexKey: fixed 4x4 grid — positions are stable
              key={`cell-${i}-${j}`}
              ...
              style={{
                backgroundColor: cell
                  ? `oklch(var(--primary) / ${Math.min(cell / 2048, 0.9)})`
                  : undefined,
              }}
            >
              {cell || ""}
